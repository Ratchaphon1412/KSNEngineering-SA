...
<div class="flex items-center justify-center py-20">
    <div class="bg-white border border-4 rounded-lg shadow relative w-1/2">
        <div class="flex items-start justify-between p-5 border-b rounded-t">
            ...
            <a href="{{ route('detail.repair.view',['repair'=>$repair  ]) }}" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center">
                <i class="fa-solid fa-xmark"></i>
            </a>
        </div>

        <div class="p-6 space-y-6">
            <form action="{{ route('repair.edit.view',['repair'=>$repair]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('POST')
