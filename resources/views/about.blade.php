<x-guest-layout>
    <div class="w-full md:h-[50vh] bg-cover bg-fixed bg-bottom bg-no-repeat shadow-xl" 
        style="background-image: url('assets/images/aboutKSN.jpg')">
        <div class="bg-black bg-opacity-20 w-full h-full py-[25%] md:py-0">
            <div class="container mx-auto flex flex-col md:justify-center md:items-center text-center h-full space-y-4">
                <div class="uppercase tracking-wide font-extrabold text-4xl text-transparent bg-clip-text bg-[#03045E]" id="type">
                    about us
                </div>
                <p class="text-white text-xl" id="intro">
                    บริษัท เค.เอส.เอ็น.เอ็นจิเนียร์ริ่ง จำกัด “ผู้นำด้าน รอก เครนโรงงาน และอุปกรณ์ช่วยยกครบวงจร”
                </p>
            </div>
        </div>
    </div>

    <div class="bg-white pb-16">
        <div class="inline-flex items-center justify-center w-full py-7">
            <hr class="w-[80%] h-px my-8 bg-gray-700 border-1 bg-black">
            <span class="absolute px-3 tracking-wide font-extrabold text-2xl text-gray-900 -translate-x-1/2 bg-white left-1/2">บริษัท เค.เอส.เอ็น.เอ็นจิเนียร์ริ่ง จำกัด</span>
        </div>
    
        <p class="w-[80%] mx-auto pl-10 tracking-wide font-bold text-lg text-gray-700 border-2 border-l-amber-500 border-t-transparent border-r-transparent border-b-transparent">
            “ขอเป็นส่วนหนึ่งในการขับเคลื่อนเศรษฐกิจของไทย 
            จากการที่ลูกค้านานาประเทศได้มาก่อตั้งบริษัทในประเทศไทยเป็นจำนวนมาก 
            ต่างให้การยอมรับภายใต้ชื่อ “บริษัท เค.เอส.เอ็น” (KSN) นำไปใช้งานในสายงานการผลิตที่ต้องการสินค้าที่มีคุณภาพและทนทาน 
            ลดความสูญเสียในกระบวนการผลิตซึ่งสิ่งที่สำคัญที่สุดคือ การบริหารเวลา ซึ่งบริษัท เข้าใจความต้องการของลูกค้า 
            โดยเราจะส่งมอบสินค้าที่มีคุณภาพให้ทันภายในเวลาที่กำหนดด้วยบริการที่รวดเร็ว”
        </p>
    </div>

    <div class="container w-[50vw] mx-auto text-center py-16 tracking-wide">
        <p class="mb-5 text-3xl font-bold text-black">
            มุ่งมั่นผลิตอุปกรณ์เคลื่อนย้ายคุณภาพสูง
        </p>
        <p class="text-md font-semibold mb-16">
            รอกโซ่ไฟฟ้า รอกสลิง รอกโยก รอกมือสาว อุปกรณ์ช่วยยก และเครนรูปแบบต่างๆ ภายใต้แบรนด์ “KSN” พัฒนาและผลิตด้วยเทคโนโลยีที่ทันสมัย สินค้าที่เปี่ยมคุณภาพและบริการจากใจ
        </p>

        <iframe class="w-[50vw] h-[60vh]" src="https://www.youtube.com/embed/XPDC7YkhwNM?si=0ToqnwkLevFzBT9i" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>

    </div>
    <hr class="border border-4 border-amber-400 mx-auto my-5">

    <div class="my-16">
        <p class="text-center font-extrabold text-3xl tracking-wide text-black">ภารกิจของบริษัท</p>
        <hr class="border border-2 border-amber-400 rounded-full w-32 mx-auto my-5">

        <div class="grid md:grid-cols-2 container mx-auto gap-10">
            <div>
                <p class="font-bold text-2xl mb-5 text-black">คุณภาพของ KSN : การขับเคลื่อนของเรา “จิตวิญญาณ” คือ “คุณภาพ”</p>
                <p class="text-gray-700">
                    KSN เชื่อว่าความเชื่อมั่นในผลิตภัณฑ์และบริการของเราคือสิ่งที่สำคัญที่สุดของบริษัท 
                    KSN มุ่งเน้นคุณภาพสูงสุดในด้านความปลอดภัย ความสะดวกในการใช้งาน 
                    ความพึงพอใจที่ลูกค้าต้องการและห่วงใยสิ่งแวดล้อม KSN 
                    ได้ส่งมอบสิ่งที่สำคัญคือ “คุณภาพ” ให้กับลูกค้าโดยผ่านจิตวิญญาณของสมาชิกและพนักงาน 
                    KSN “ทุกอย่างเพื่อความพึงพอใจของลูกค้า”
                </p>
            </div>
            <div>
                <p class="font-bold text-2xl mb-5 text-black">ภารกิจของ KSN : การส่งมอบความพึงพอใจสูงสุดให้กับลูกค้า</p>
                <p class="text-gray-700">
                มุ่งเน้นความปลอดภัยและสิ่งที่เป็นประโยชน์ต่อการดำเนินงานของลูกค้า 
                โดยจัดหาสินค้าและบริการด้วความคิดสร้างสรรค์และเทคโนโลยีที่ทันสมัย KSN 
                จะใช้มุมมองของลูกค้าในการออกแบบเสมอและนำนวัตกรรมใหม่มาใช้ในการจัดการอุตสาหกรรม 
                คุณค่าของ KSN “ความไว้วางใจจากลูกค้า” คือ สิ่งสำคัญที่สุดของเรา KSN 
                ได้ตั้งเป้าหมายความสำเร็จขึ้นอยู่กับภาระกิจนี้และเชื่อว่าจะนำไปสู่ความเจริญรุ่งเรืองของพนักงาน KSN และผู้ที่มีส่วนเกี่ยวข้อง
                </p>
            </div>
        </div>
    </div>

    <div class="bg-[#474747]">
        <div class="container mx-auto grid md:grid-cols-2 gap-4 py-16">
                <img src="assets/images/goKSN.jpg" alt="">
                <ol class="text-white font-semibold tracking-wide place-self-center">
                    <p class="text-2xl text-amber-400 font-bold tracking-wide mb-4">เป้าหมายในอนาคต</p>
                    <li> 1. มุ่งมั่นก้าวไปสู่ส่วนแบ่งทางการตลาดของรอกโซ่ไฟฟ้าเป็นอันดับ 1 ในประเทศไทย</li>
                    <li> 2. ตอบสนองทุกความต้องการของลูกค้า </li>
                    <li> 3. เพิ่มคุณภาพของผลิตภัณฑ์ให้ได้มาตรฐานและคุณภาพที่สูงขึ้น </li>
                    <li> 4. ซื่อสัตย์และสร้างความไว้วางใจให้กับลูกค้าอย่างดีที่สุด </li>
                    <li> 5. เอาใจใส่งานบริการเพื่อความเชื่อมั่นของลูกค้า</li>
                    <li> 6. ก้าวไปสู่เวทีระดับโลกตามเป้าหมายในอนาคตของบริษัทแม่ที่ญี่ปุ่น </li>
                </ol>
        </div>
    </div>

    <style scoped>
    #type{
        width: 8ch;
        animation: typing 1s steps(8), blink .5s step-end infinite alternate;
        white-space: nowrap;
        overflow: hidden;
        border-right: 3px solid;
    }

    @keyframes typing{
        from {
            width: 0
        }
    }

    @keyframes blink {
        50%{
            border-color: transparent
        }
    }

    #intro{
        animation: intro 1.5s;
    }

    @keyframes intro{
        0% {
        transform: translateY(25%);
    }
    100% {
        transform: translateY(0);
    }
    }

    </style>
    
</x-guest-layout>